...
def main(
    ...
    parser.add_argument("--record_timeout", default=8, help="How real time the recording is in seconds.", type=float)
    parser.add_argument("--phrase_timeout", default=8, help="How much empty space between recordings before we consider it a new line in the transcription.", type=float)
    ...
    if "linux" in platform:
        ...
        if not mic_name or mic_name == "list":
            ...
            for index, name in enumerate(sr.Microphone.list_microphone_names()):
                print(f'Micrdophone with name "{name}" found')
            return
        else:
            for index, name in enumerate(sr.Microphone.list_microphone_names()):
                if mic_name in name:
